namespace SRDebugger.Services.Implementation
{
    ...
    public class KeyboardShortcutListenerService : SRServiceBase<KeyboardShortcutListenerService>
    {
        ...
        protected override void Update()
        {
            ...
            if (Settings.Instance.KeyboardEscapeClose && Keyboard.current.escapeKey.wasPressedThisFrame && Service.Panel.IsVisible)
            {
                SRDebug.Instance.HideDebugPanel();
            }

            var ctrl = Keyboard.current.leftCtrlKey.isPressed || Keyboard.current.rightCtrlKey.isPressed;
            var alt = Keyboard.current.leftAltKey.isPressed || Keyboard.current.rightAltKey.isPressed;
            var shift = Keyboard.current.leftShiftKey.isPressed || Keyboard.current.rightShiftKey.isPressed;
            ...
            for (var i = 0; i < _shortcuts.Count; i++)
            {
                ...
                if (((KeyControl)Keyboard.current[s.Key.ToString()]).wasPressedThisFrame)
                {
                    ExecuteShortcut(s);
                    break;
                }
